...
class BaseRelocationTable: DataDirectoryElement {
    override val data: QWord
    override val realOffset: QWord
    override val peType: PeType

    override val hex: String get() = data.hex
    override val realName = "Base Relocation Table [.reloc]"
    ...
    constructor(headerOffset: QWord, magic: Word, file: RandomAccessFile) {
        ...
        peType = when (magic) {
            PeType.Companion.PE32_MAGIC -> PeType.Pe32(136, size)
            PeType.Companion.PE32_PLUS_MAGIC -> PeType.Pe32Plus(152, size)
    ...
    constructor(original: BaseRelocationTable, newData: QWord) {
        data = newData
